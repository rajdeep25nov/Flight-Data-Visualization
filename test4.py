#this code is for plotting a 3D flight trajectory animation using Plotly
#     in this code we we dint add hover tooltips to display detailed information about each point in the flight path.
import pandas as pd
...
try:
    ...
    print("Errorrrr dev: file not found bro maybe wrong path, huhh silly mistake.")
    exit()
...
for fmt in time_formats:
    try:
        ...
        if df['time'].notna().sum() > 0:
            print(f"Good Dude, Successfully parsed time with format: {fmt}")
            break
    except Exception as e:
        print(f"Failed to parse with format {fmt}: {e}")
else:
    print("Warninggg: All time parsing attempts failed. Using original data without time sorting.")
...
for i in frame_indices:
    frames.append(go.Frame(
        data=[
            ...
            go.Scatter3d(  # Aircraft marker at current position
...
fig = go.Figure(
    data=[
        start_marker,
        end_marker,
        go.Scatter3d(  # Initial trajectory
        ...
        aircraft_marker  # Initial aircraft marker
    ],
    layout=go.Layout(
        title="3D Flight Trajectory Animation with Aircraft",
        scene=dict(
...
print("Happy Happy Happy....Rendering plot...")
fig.show()